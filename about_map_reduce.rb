...
class AboutMapReduce < EdgeCase::Koan
  ...
  def test_map_reduce
    ...
    assert_equal 6.0, @comments.map_reduce(map, reduce).find.first['value']['votes']
  end
  ...
  def test_map_reduce_with_query
    ...
    # assert_equal "\{"votes"=>5.0\}", @comments.map_reduce(map, reduce, {:query => {:votes => {'$gt' => 1}}}).find.first['value']
    assert_equal 5.0, @comments.map_reduce(map, reduce, {:query => {:votes => {'$gt' => 1}}}).find.first['value']['votes']
  end
